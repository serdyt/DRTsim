...
#        self.logger = logging.FileHandler(self.scope)
#        self.logger.setLevel(logging.DEBUG)
#        frmt = logging.Formatter('%(asctime)s,%(name)s,%(levelname)s,%(message)s')
#        self.logger.setFormatter(frmt)

        print(self.scope)
        self.logger = self.setup_logger(name = self.scope, log_file = self.scope)


    @property
    def reservoir_pct(self):
        """
        Returns current reservour level in percents
        """
    ...
    def _car(self, i):
        ...
        with self.fuel_pumps.request() as pump_req:
            self.logger.info('car{} awaiting pump'.format(i))
            yield pump_req
            self.logger.info('car{} at pump'.format(i))
            ...
            self.logger.info('car{} pumped {}L in {:.0f}s'.format(
                i, amount, pump_time))
...
config = {
    ...
    'sim.log.format': '{level:4} {ts:.3f} {ts_unit}: {scope:<16}:',
    'sim.log.level': 'DEBUG',
