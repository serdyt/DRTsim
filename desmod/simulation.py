...
class SimEnvironment(simpy.Environment):
    ...
    def __init__(self, config):
        ...
        #: database with input data
        self.db = sqlite3.connect(self.config.setdefault('db.file',':memory:'))
    ...
    def __init__(self, config):
        self.workspace = config.setdefault('meta.sim.workspace',
                                           config.setdefault('sim.workspace',
                                                             os.curdir))
        self.overwrite = config.setdefault('sim.workspace.overwrite', True)
...
def simulate(config, top_type, env_type=SimEnvironment, reraise=True,
    ...
    try:
        with _Workspace(config):
            env = env_type(config)
            # Python logging is used instead of tracers
